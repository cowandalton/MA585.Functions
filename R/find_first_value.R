...
#' 
#' This function finds the value of the first parameter for a function that gives a desired value.
...
#' @return A list will be returned containing the estimate for the first parameter, the function value when the parameter estimate is used in f, and a warning if the parameter estimate results in a function value that is more than 1 away from val.
#' @export 
#' @examples 
#' find_first_value(log, val = 6.3, maximum = 10000)
#' find_first_value(sqrt, val = 17.8432)

find_first_value <- function(f, val = 0, maximum = 100000, minimum = 0, only_integers = F, decimals = 4){
  ...
  min_check <- try(f(minimum), silent = T)
  ...
  max_check <- try(f(maximum), silent = T)
  ...
  for(i in -10:9){
    ...
    if(i*10^(digits-1) < minimum){
      current_value <- f(minimum)
    } else{
      current_value <- f(i*10^(digits-1))
    }
  ...
  for(j in 2:(totdigits-1)){
    ...
    for(i in 0:9){
      ...
      if(value+i*10^(digits-j) < minimum){
        ...
        current_value <- f(value+i*10^(digits-j))
      }
      if(value+(i+1)*10^(digits-j) > maximum){
        next_value <- f(maximum)
      } else{
        next_value <- f(value+(i+1)*10^(digits-j))
      }
      if((current_value >= val) & (next_value <= val)){
  ...
  for(i in 0:9){
    ...
    if(value+i*10^(digits-totdigits) < minimum){
      ...
      current_value <- f(value+i*10^(digits-totdigits))
    }
    if(value+(i+1)*10^(digits-totdigits) > maximum){
      next_value <- f(maximum)
    } else{
      next_value <- f(value+(i+1)*10^(digits-totdigits))
    }
  ...
  greater_value <- try(f(value+1), silent = T)
